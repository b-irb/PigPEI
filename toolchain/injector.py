...
def main(fname: str, fv_path: str, new_fv_path: str):
    ...
    print(f"decompressing LZMA section ({compressed_section.size} bytes)")
    try:
    ...
    MAX_COMPRESSION_SIZE = compressed_section.size - calcsize(FileSectionFmt)
    if len(compressed) > MAX_COMPRESSION_SIZE:
        sys.exit(f"new compressed section is too large (max {MAX_COMPRESSION_SIZE} bytes)")
